...
class FlamesOfGalakrond
{
    private:
        ...
        {
            FlamesData() :
                spellAreaTriggerGuid(0), targetGuid(0), targetsHitCount(0)
            {

            }

            FlamesData(uint64 triggerGuid, uint64 target) :
                spellAreaTriggerGuid(triggerGuid), targetGuid(target), targetsHitCount(0)
            {
            }

            uint64 spellAreaTriggerGuid;
            uint64 targetGuid;
    ...
    public:
        ...
        void AddFlame(uint64 triggerGuid, uint64 targetGuid)
        {
        ...
        void RemoveFlame(uint64 triggerGuid)
        {
            std::map<uint64, FlamesData>::iterator itr = m_Flames.find(triggerGuid);
            if (itr != m_Flames.end())
            {
                m_Flames.erase(itr);
            }
        ...
        void FlameHitTarget(uint64 triggerGuid)
        {
            if (m_Flames.find(triggerGuid) == m_Flames.end())
                return;
        ...
        uint32 GetFlameHitCount(uint64 triggerGuid)
        {
            if (m_Flames.find(triggerGuid) == m_Flames.end())
                return 0;
    ...
    private:
        ...
        std::map<uint64, FlamesData> m_Flames;
...
class boss_galakras : public CreatureScript
{
    public:
        ...
        struct boss_galakrasAI : public BossAI
        {
            ...
            {
                if (id == DATA_LAST_FLAMES_OF_GALAKROND_TARGET)
                {
        ...
        private:
            ...
            void ProcessDown()
            {
                ...
                if (Creature* pZaela = instance->instance->GetCreature(instance->GetData64(DATA_WARLORD_ZAELA)))
                {
            ...
            void MoveCustomFall()
            {
                ...
                init.MoveTo(fallPos.GetPositionX(), fallPos.GetPositionY(), fallPos.GetPositionZ());
                init.SetFall();
                init.Launch();
            }
        ...
        private:
            ...
            uint64 m_LastFlamesOfGalakrondGuid;
...
class npc_galakras_korkron_elite_sniper : public CreatureScript
{
    public:
        ...
        private:
            ...
            {
                Player* player = me->FindNearestPlayer(100.0f, true);
...
struct npc_galakras_tower_defender : public ScriptedAI
{
    ...
    protected:
        ...
        Creature* GetGalakras()
        {
            if (pInstance)
            {
                return pInstance->instance->GetCreature(pInstance->GetData64(DATA_GALAKRAS));
            }
        ...
        private:
            ...
            void CheckCreatures(const uint32 diff)
            {
                ...
                if (checkCreaturesTimer <= diff)
                {
                    ...
                    for (std::set<uint64>::iterator itr = creatureGuids.begin(); itr != creatureGuids.end();)
                    {
                        uint64 creatureGuid = (*itr);

                        Unit* creature = Unit::GetUnit(*me, creatureGuid);
                        if (!creature)
                        {
            ...
            void StopApplyingAuras()
            {
                ...
                for (std::set<uint64>::iterator itr = creatureGuids.begin(); itr != creatureGuids.end(); ++itr)
                {
                    uint64 creatureGuid = (*itr);
...
class npc_galakras_tower : public CreatureScript
{
    public:
        ...
        private:
            ...
            void SelectPlayers(const uint32 diff)
            {
                if (playersSelectTimer <= diff)
                {
                    ...
                    for (std::set<uint64>::iterator itr = m_PlayersInTower.begin(); itr != m_PlayersInTower.end();)
                    {
            ...
            bool IsPlayerInTower(uint64 guid) const
            {
        ...
        private:
            ...
            std::set<uint64> m_PlayersInTower;
    ...
    public:
        ...
        class spell_galakras_tower_strength_AuraScript : public AuraScript
        {
            ...
            void HandlePeriodicTick(AuraEffect const* aurEff)
            {
                ...
                Creature* caster = Unit::GetCreature(*owner, casterGuid);
                if (!caster)
                {
                    Remove();
                    return;
...
class spell_area_galakras_flame_arrows_dmg : public SpellAreaTriggerScript
{
    public:
        spell_area_galakras_flame_arrows_dmg() : SpellAreaTriggerScript("spell_area_galakras_flame_arrows_dmg") {}
        ...
        bool OnAddTarget(AreaTrigger* trigger, Unit* target)
        {
            ...
            trigger->GetCaster()->AddAura(SPELL_FLAME_ARROWS_PERIODIC, target);
...
class spell_area_galakras_flames_of_galakrond : public SpellAreaTriggerScript
{
    public:
        spell_area_galakras_flames_of_galakrond() : SpellAreaTriggerScript("spell_area_galakras_flames_of_galakrond") {}
        ...
        void OnCreate(AreaTrigger* trigger) override
        {
            ...
            uint64 targetGuid = pCreature->AI()->GetGUID(DATA_LAST_FLAMES_OF_GALAKROND_TARGET);
            ...
            {
                galakrasAI->GetFlamesOfGalakrond().AddFlame(trigger->GetGUID(), targetGuid);
            }
        }
        ...
        bool OnAddTarget(AreaTrigger* trigger, Unit* target)
        {
            ...
            if (Creature* pCreature = caster->ToCreature())
            {
                if (GalakrasAI* galakrasAI = CAST_AI(GalakrasAI, pCreature->GetAI()))
                {
                    galakrasAI->GetFlamesOfGalakrond().FlameHitTarget(trigger->GetGUID());
                }
            }
        ...
        void OnRemove(AreaTrigger* trigger, bool isExpired) override
        {
            Unit* caster = trigger->GetCaster();
            if (!caster)
                return;
            ...
            if (GalakrasAI* galakrasAI = CAST_AI(GalakrasAI, pCreature->GetAI()))
            {
                hitCount = galakrasAI->GetFlamesOfGalakrond().GetFlameHitCount(trigger->GetGUID());
            }

            // We need to change spell damage and cast spell with position
            // there is no default function to to it

            SpellInfo const* l_SpellInfo = sSpellMgr->GetSpellInfo(SPELL_FLAMES_OF_GALAKROND_DMG, pCreature->GetMap()->GetDifficulty());
            if (!l_SpellInfo)
                return;
...
void AddSC_boss_galakras()
{
    ...
    new spell_area_galakras_flame_arrows_dmg();     // 146764
    new spell_area_galakras_flames_of_galakrond();  // 146991
}