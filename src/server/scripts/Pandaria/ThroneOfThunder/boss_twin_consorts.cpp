    ...
    public:
        ...
        struct boss_lulinAI : public BossAI
        {
            ...
            void Reset()
            {
                ...
                DespawnCreaturesInArea(NPC_MOON_LOTUS_2, me);
                DespawnCreaturesInArea(NPC_STAR, me);
                
                if (Creature* pSuen = instance->instance->GetCreature(instance->GetData64(DATA_SUEN)))
                {
                    if (!pSuen->isAlive())
                    {
                        pSuen->Respawn();
            ...
            {
                if (type == DATA_SUMMONED_LOTUS)
                {
                    return summonedLotuses;
                }
    ...
    public:
        ...
        struct boss_suenAI : public BossAI
        {
            ...
            void Reset()
            {
                ...
                if (Creature* pLulin = instance->instance->GetCreature(instance->GetData64(DATA_LULIN)))
                {
                    if (!pLulin->isAlive())
                    {
                        pLulin->Respawn();
...
class npc_lulin_moon_lotus_2 : public CreatureScript
{
    public:
        ...
        private:
            ...
            void CheckAndCastSpores(const uint32 diff)
            {
                if (sporeTimer <= diff)
                {
                    sporeTimer = 1000;
                    ...
                    if (!players.empty())
                    {
                        ...
                        {
                            if (targetGuids.find((*itr)->GetGUID()) == targetGuids.end())
                            {
                                targetGuids.insert((*itr)->GetGUID());
                                DoCast((*itr), SPELL_SLUMBER_SPORES, true);
        ...
        private:
            ...
            void CheckFaceSuen(const uint32 diff)
            {
                if (faceTimer <= diff)
                {
                    ...
                    if (Creature* pSuen = me->FindNearestCreature(NPC_SUEN, 200.0f))
                    {
        ...
        private:
            ...
            void PlayerCheck(const uint32 diff)
            {
                ...
                if (playerCheckTimer <= diff)
                {
                    ...
                    if (Player* player = me->FindNearestPlayer(2.0f))
                    {
                        isActivated = true;
    ...
    public:
        ...
        class spell_lulin_tidal_force_AuraScript : public AuraScript
        {
            ...
            void HandleTick(AuraEffect const* aurEff)
            {
                ...
                Position pos;
                GetUnitOwner()->GetNearPoint2D(pos.m_positionX, pos.m_positionX, 80.0f, frand(0, 2 * M_PI));
...
class spell_area_suen_flames_of_passion : public SpellAreaTriggerScript
{
    public:
        spell_area_suen_flames_of_passion() : SpellAreaTriggerScript("spell_area_suen_flames_of_passion") {}
        ...
        bool OnAddTarget(AreaTrigger* trigger, Unit* target)
        {
            ...
            trigger->GetCaster()->AddAura(SPELL_FLAMES_OF_PASSION_DMG_2, target);
...
class spell_area_lulin_icy_shadows : public SpellAreaTriggerScript
{
    public:
        spell_area_lulin_icy_shadows() : SpellAreaTriggerScript("spell_area_lulin_icy_shadows") {}
        ...
        bool OnAddTarget(AreaTrigger* trigger, Unit* target)
        {
            ...
            if (!CanHitTarget(trigger, target))
                return false;

            trigger->GetCaster()->AddAura(SPELL_ICY_SHADOWS_DEBUFF, target);
        ...
        bool OnRemoveTarget(AreaTrigger* trigger, Unit* target)
        {
            ...
            target->GetAreaTriggerList(triggers, SPELL_ICY_SHADOWS_AREATRIGGER_1);

            if (!triggers.empty())
            {
                for (std::list<AreaTrigger*>::const_iterator itr = triggers.begin(); itr != triggers.end(); ++itr)
                {
                    if (CanHitTarget((*itr), target))
                        return true;
                }
            }

            target->RemoveAura(SPELL_ICY_SHADOWS_DEBUFF, trigger->GetCasterGUID());

            return true;
        }

        void OnUpdate(AreaTrigger* trigger)
        {
            if (!trigger)
                return;

            std::set<uint64>* targets = trigger->GetTargets();
            for (std::set<uint64>::const_iterator itr = targets->begin(); itr != targets->end();)
            {
                if (Unit* target = Unit::GetUnit(*trigger, (*itr)))
                {
                    if (!CanHitTarget(trigger, target))
                    {
                        target->RemoveAura(SPELL_ICY_SHADOWS_DEBUFF);
    ...
    private:
        ...
        bool CanHitTarget(AreaTrigger* trigger, Unit* target)
        {
            if (trigger->GetDistance(target) >= 10.0f || !trigger->isInBack(target, M_PI / 4))
            {
                return false;
            }
...
void AddSC_boss_twin_consorts()
{
    ...
    new spell_area_lulin_icy_shadows();         // 137725 137439
}
