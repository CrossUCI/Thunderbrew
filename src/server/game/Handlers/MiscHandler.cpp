...
void WorldSession::HandleWhoOpcode(WorldPacket& recvData)
{
    ...
    std::vector<uint32> unkLens(patternsCount);

    for (uint8 i = 0; i < patternsCount; i++)
        unkLens.push_back(recvData.ReadBits(7));
...
{
    if (!GetPlayer() || !GetPlayer()->GetSocial())
        return;
...
void WorldSession::SendAreaTriggerDenied(uint32 trigger_id)
{
    bool unkbit = false;

    WorldPacket data(SMSG_AREA_TRIGGER_DENIED, 5);
    data << uint32(trigger_id);
...
void WorldSession::HandleAreaTriggerOpcode(WorldPacket& recvData)
{
    ...
    {
        sLog->outDebug(LOG_FILTER_NETWORKIO, "HandleAreaTriggerOpcode: Player '%s' (GUID: %u) in spectator mode, ignore Area Trigger ID:%u",
            player->GetName(), player->GetGUIDLow(), triggerId);
        return;
    }
    ...
    if (sScriptMgr->OnAreaTrigger(player, atEntry, l_Enter))
        return;
    ...
    if (player->GetMapId() != at->target_mapId)
    {
        ...
        {
            SendAreaTriggerDenied(triggerId);
            return;
        }
    ...
    if (GetPlayer()->GetZoneId(true) == 616)
...
void WorldSession::HandleSetActionButtonOpcode(WorldPacket& recvData)
{
    ...
    uint32 type = ACTION_BUTTON_TYPE(packetData);
    ...
    else
    {
        switch (type)
        {
            ...
            case ACTION_BUTTON_PET:
                sLog->outInfo(LOG_FILTER_NETWORKIO, "MISC: Added Battle pet %u into button %u", action, button);
                break;
            default:
...
void WorldSession::HandleComplainOpcode(WorldPacket& recvData)
{
    ...
    uint32 complaintType = recvData.read<uint8>();
    ...
    WorldPacket data(SMSG_COMPLAIN_RESULT, 5);
    data << uint32(complaintType);  // value 0xC generates a "CalendarError" in client. // value
    data << uint8(0);   // value 1 resets CGChat::m_complaintsSystemStatus in client. (unused?) // result
    SendPacket(&data);
}
...
void WorldSession::HandleRequestHotfix(WorldPacket& p_RecvData)
{
    uint32 l_Type, l_Cout;
    p_RecvData >> l_Type;

    l_Cout = p_RecvData.ReadBits(21);

    ObjectGuid* l_Guids = new ObjectGuid[l_Cout];

    uint8 l_BitsOrder[8] = { 2, 4, 3, 6, 7, 1, 5, 0 };
    for (uint32 l_Iter = 0; l_Iter < l_Cout; ++l_Iter)
        p_RecvData.ReadBitInOrder(l_Guids[l_Iter], l_BitsOrder);

    uint32 l_Entry;
    for (uint32 l_Iter = 0; l_Iter < l_Cout; ++l_Iter)
    {
        p_RecvData.ReadByteSeq(l_Guids[l_Iter][5]);
        p_RecvData.ReadByteSeq(l_Guids[l_Iter][4]);
        p_RecvData.ReadByteSeq(l_Guids[l_Iter][3]);
        p_RecvData >> l_Entry;
        p_RecvData.ReadByteSeq(l_Guids[l_Iter][7]);
        p_RecvData.ReadByteSeq(l_Guids[l_Iter][0]);
        p_RecvData.ReadByteSeq(l_Guids[l_Iter][2]);
        p_RecvData.ReadByteSeq(l_Guids[l_Iter][1]);
        p_RecvData.ReadByteSeq(l_Guids[l_Iter][6]);
...
{
    switch (sLFGMgr->GetState(GetPlayer()->GetGUID()))
    {
    case LfgState::LFG_STATE_FINISHED_DUNGEON:
    case LfgState::LFG_STATE_ROLECHECK:
    case LfgState::LFG_STATE_RAIDBROWSER:
    case LfgState::LFG_STATE_NONE:
        break;
    default:
        return;
    }

    uint32 factionId = recvPacket.read<uint32>();
...
void WorldSession::HandleTradeInfo(WorldPacket& recvPacket)
{
    ...
    skill.SkillValue = plr->GetSkillValue(skillId);
    skill.SkillMaxValue = plr->GetMaxSkillValue(skillId);
    Skills.push_back(skill);

    for (uint32 i = 0; i < sSkillLineStore.GetNumRows(); ++i)
    {
        if (SkillLineEntry const* proto = sSkillLineStore.LookupEntry(i))
        {
            if (proto->parentSkillLineID == skillId)
            {
                if (plr->HasSkill(proto->id))
                {
                    SkillInfo subSkill;
                    subSkill.SkillId = proto->id;
                    subSkill.SkillValue = plr->GetSkillValue(proto->id);
                    subSkill.SkillMaxValue = plr->GetMaxSkillValue(proto->id);

                    Skills.push_back(subSkill);
                }
            }
        }
    }
    ...
    for (auto itr : plr->GetSpellMap())
    {
        ...
        for (SkillInfo const& info : Skills)
        {
            if (!spell->IsAbilityOfSkillType(info.SkillId))
                continue;
    ...
    data << uint32(spellId);
    data.WriteBits(Skills.size(), 22); // skill max value
    data.WriteBits(Spells.size(), 22); // spell count
    data.WriteBit(guid[2]);
    data.WriteBit(guid[6]);
    data.WriteBit(guid[1]);
    data.WriteBit(guid[4]);
    data.WriteBits(Skills.size(), 22); // skill id count
    data.WriteBit(guid[7]);
    data.WriteBit(guid[0]);
    data.WriteBit(guid[5]);
    data.WriteBits(Skills.size(), 22); // skill value count
    ...
    for (SkillInfo const& info : Skills)
    {
        data << uint32(info.SkillMaxValue);
    }
    for (SkillInfo const& info : Skills)
    {
