...
class BattlegroundQueue
{
    public:
        ...
        void SetQueueTypeId(BattlegroundQueueTypeId bgQueueTypeId) { m_queueTypeId = bgQueueTypeId; }
        BattlegroundQueueTypeId GetQueueTypeId() const { return m_queueTypeId; }

        void BattlegroundQueueUpdate(uint32 diff, BattlegroundTypeId bgTypeId, BattlegroundBracketId bracket_id, uint8 arenaType = 0, bool isRated = false, uint32 minRating = 0, uint32 const* disabledBGs = nullptr);
        void UpdateEvents(uint32 diff);

        bool SelectArenaTeams(uint32 bracket_id, uint32 arenaRating, GroupQueueInfo* & team1, GroupQueueInfo* & team2);

        void FillPlayersToBG(Battleground* bg, BattlegroundBracketId bracket_id);
        bool CheckPremadeMatch(BattlegroundBracketId bracket_id, uint32 MinPlayersPerTeam, uint32 MaxPlayersPerTeam);
        bool CheckNormalMatch(Battleground* bg_template, BattlegroundBracketId bracket_id, uint32 minPlayers, uint32 maxPlayers);
        bool CheckSkirmishForSameFaction(BattlegroundBracketId bracket_id, uint32 minPlayersPerTeam);
        GroupQueueInfo* AddGroup(Player* leader, Group* group, BattlegroundTypeId bgTypeId, PvPDifficultyEntry const*  bracketEntry, uint8 ArenaType, bool isRated, bool isPremade, uint32 ArenaRating, uint32 MatchmakerRating, uint32 const* disabledBGs = nullptr);
        ...
        */
        ...
        public:
            ...
            void Init();
            bool AddGroup(GroupQueueInfo* ginfo, uint32 desiredCount, uint32 mapId = 0);
