...
void ReputationMgr::SendInitialReputations()
{
    ...
    for (FactionStateList::iterator itr = _factions.begin(); itr != _factions.end(); ++itr)
    {
        ...
        bits.WriteBit(_player->GetSession()->GetReputationModifier(itr->second.ID) != 0);
...
void ReputationMgr::SendFactionBonusInfo()
{
    ...
    for (FactionStateList::iterator itr = _factions.begin(); itr != _factions.end(); ++itr)
    {
        ...
        data.WriteBit(_player->GetSession()->GetReputationModifier(itr->second.ID) != 0);
        ++a;
    }
...
bool ReputationMgr::SetOneFactionReputation(FactionEntry const* factionEntry, int32 standing, bool incremental)
{
    ...
    if (itr != _factions.end())
    {
        ...
        if (incremental)
        {
            ...
            standing = int32(floor((float)standing * _player->GetRate<float>(RateType::ReputationGain) + 0.5f));
...
void ReputationMgr::LoadAfterInterRealm(QueryResult result)
{
    if (sWorld->GetInterRealmSession()->GetProtocolVersion() >= PROTOCOL_WITH_REPUTATION)
    {
        return;
    }
