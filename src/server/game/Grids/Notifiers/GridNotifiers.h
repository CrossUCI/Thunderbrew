...
namespace JadeCore
{
    ...
    class AnyFriendlyUnitInObjectRangeCheck
    {
        public:
            ...
            bool operator()(Unit* u)
            {
                if (u && u->isAlive() && i_obj->IsWithinDistInMap(u, i_range) && i_funit->IsFriendlyTo(u))
                    return true;
                else
                    return false;
            }
    ...
    class AnyFriendlyPlayerOrPetInObjectRangeCheck
    {
        public:
            AnyFriendlyPlayerOrPetInObjectRangeCheck(WorldObject const* obj, Unit const* funit, float range) : i_obj(obj), i_funit(funit), i_range(range) {}
            bool operator()(Unit* u)
            {
                if (!u->ToPlayer() && !u->ToPet())
                    return false;
    ...
    class MostHPMissingInRangePL
    {
        public:
            MostHPMissingInRangePL(Player* player, float range, uint32 hp) : i_obj(player), i_range(range), i_hp(hp) {}
            bool operator()(Player* u)
            {
                if (u->isAlive() && u->isInCombat() && !i_obj->IsHostileTo(u) && i_obj->IsWithinDistInMap(u, i_range) && u->GetMaxHealth() - u->GetHealth() > i_hp)
                {
    ...
    */
    ...
    // Success at unit in range, range update for next check (this can be use with CreatureLastSearcher to find nearest creature)
    class NearestSummonedCreatureEntryWithLiveStateInObjectRangeCheck
    {
        public:
            NearestSummonedCreatureEntryWithLiveStateInObjectRangeCheck(WorldObject const& obj, uint32 entry, bool alive, float range, uint64 ownerGuid)
                : i_obj(obj), i_entry(entry), i_alive(alive), i_range(range), i_ownerGuid(ownerGuid) {}

            bool operator()(Creature* u)
            {
                if (u->GetEntry() == i_entry && u->isAlive() == i_alive && i_obj.IsWithinDistInMap(u, i_range))
                {
                    if (u->GetOwnerGUID() != i_ownerGuid)
                        return false;
        ...
        private:
            ...
            // prevent clone this object
            NearestSummonedCreatureEntryWithLiveStateInObjectRangeCheck(NearestSummonedCreatureEntryWithLiveStateInObjectRangeCheck const&);
    };
