...
void SocialMgr::GetFriendInfo(Player* player, uint32 friendGUID, FriendInfo &friendInfo)
{
    ...
    Player* pFriend = ObjectAccessor::FindPlayer(MAKE_NEW_GUID(friendGUID, 0, HighGuid::Player));
...
void SocialMgr::MakeFriendStatusPacket(FriendsResult result, uint32 guid, WorldPacket* data)
{
    ...
    *data << uint8(result);
    *data << uint64(MAKE_NEW_GUID(guid, 0, HighGuid::Player));
}
...
void SocialMgr::UpdateFriendList(Player* player)
{
    if (!player)
        return;
    ...
    for (SocialMap::const_iterator itr = m_socialMap.begin(); itr != m_socialMap.end(); ++itr)
    {
        ...
        if (itr2 != itr->second.m_playerSocialMap.end() && (itr2->second.Flags & SOCIAL_FLAG_FRIEND))
        {
            ...
            if (pFriend && pFriend->IsInWorld() &&
                ...
                player->IsVisibleGloballyFor(pFriend))
            {
                if (auto l_Social = pFriend->GetSocial())
                    l_Social->SendSocialList(pFriend);
            }
        }
    }
}
